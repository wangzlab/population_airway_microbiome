...
# Colnames: remove special symbols in the colnames 
colnames(bact.features.st) <- gsub("\\W",".", colnames(bact.features.st))
colnames(fung.features.st) <- gsub("\\W",".", colnames(fung.features.st))
colnames(metagMod.features.st) <- gsub("\\W",".", colnames(metagMod.features.st))
colnames(ARG.features.st) <- gsub("\\W",".", colnames(ARG.features.st))
colnames(VF.features.st) <- gsub("\\W",".", colnames(VF.features.st))
  ...
  for(lf in LungFuns){
     ...
     for(mf.dn in c("bact.features.st", "fung.features.st", "metagMod.features.st", "ARG.features.st", "VF.features.st")){
       ...
       for(i in 1:ncol(mf.data)){
         ...
         summary <- 
           tryCatch({
           summary(mediate(model.m,model.y,treat=exp,mediator=mf,boot=F,sims=1000))
         }, error = function(e) {
           "error"  # if the above code generates error, summary = F
         })
         ...
         summary =  tryCatch({
           summary(mediate(model.m,model.y,treat=exp,mediator=lf,boot=F,sims=1000))
         }, error = function(e) {
           "error"  # if the above code generates error, summary = F
         })
